...
│   ├── pnl_logger.py        # Daily P&L reporter — DailyPnLLogger daemon thread writes net P&L per symbol to logs/pnl.log at UTC midnight
...
- **Daily P&L reporting.** `DailyPnLLogger` in `utils/pnl_logger.py` runs as a daemon thread started in `_run()`. It sleeps until 00:00:05 UTC, then calls `account.get_futures_trades_for_range` for each configured symbol over the just-completed UTC day (Binance day boundaries align with UTC midnight). Net P&L (realized_pnl − commission) is written per symbol and as a total to `logs/pnl.log` (configured via `logging.pnl_log_file`). The file is plain-appended — one report block per day — and is separate from loguru sinks so it reads as a clean ledger. A one-line summary is also emitted to the main loguru log. Fetches up to 1000 trades per symbol per request (Binance cap); sufficient for this bot's trade frequency.

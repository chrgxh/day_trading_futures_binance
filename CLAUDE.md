...
- **Two-stage limit entry.** Positions are opened via `attempt_limit_entry` in `bot.py` using a GTX-first, IOC-chase fallback strategy. Stage 1: GTX (post-only) limit at the current best bid (BUY) or best ask (SELL) — up to `entry.gtx_attempts` (default 3) attempts of `entry.gtx_timeout_secs` (default 5s) each. An instant GTX rejection (price already crossing the spread) counts as one attempt and moves to the next immediately without waiting. Stage 2: if all GTX attempts are exhausted (via rejection or timeout), the bot switches to an IOC chase — it repeatedly places IOC limit orders at the current best ask (BUY) or best bid (SELL) until filled. The only abort condition for either stage is price drifting beyond `entry.max_price_deviation_pct` (default 0.3%) from the signal candle close price; strategy criteria cannot change mid-candle since signals only update on closed candles. Stop losses and trailing TP are anchored to the actual fill price, not the signal price. The crossover candle close price is passed through `TradeSignal.entry_price`; strategies that don't set it fall back to a fresh mark price fetch.
...
- **Interval-driven candle limit and period scaling.** `trading.interval` is the single field to change when switching candle size. The candle prefetch count is auto-computed as `(200 × 60 / interval_minutes) + 50` to always cover 200 complete 1h bars for the trend EMA. If `trading.candle_limit` is set explicitly in `config.yaml` it overrides the auto-computed value. `get_futures_ohlcv` paginates automatically when the required count exceeds Binance's 1500-candle-per-request cap. All period-based strategy params (fast/slow EMA, RSI, RVOL lookback) are scaled by `reference_interval / current_interval` so time coverage is preserved; the trend EMA period (1h-based) is not scaled. `interval_to_minutes` in `utils/indicators.py` performs the interval string → minutes conversion.
- **Position reconciliation on every tick.** Before each strategy evaluation, `bot.py` calls `account.get_futures_positions` for the symbol and compares the result to the in-memory `open_positions` dict. If they differ (e.g. a stop-loss fired silently on Binance), a WARNING is logged and the in-memory state is corrected before the strategy runs. This removes the need for a user data stream for basic state consistency.
...
- **Dual stop losses on every open.** Immediately after a limit entry fills, `execute_signal` places two conditional orders: a stop-limit at `risk.stop_loss_limit_pct` (preferred exit, less slippage) and a stop-market at `risk.stop_loss_market_pct` (safety net if price gaps past the limit). Both use `reduceOnly=True`. When the strategy signals `CLOSE`, both are cancelled before `close_position` is called. Stop order IDs are held in memory only — on restart they are not recovered, so the orders remain live on Binance but the bot will not cancel them on a strategy-driven close. Query open algo orders manually after an unclean restart.
